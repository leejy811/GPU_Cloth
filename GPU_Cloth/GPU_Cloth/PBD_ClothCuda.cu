...
void PBD_ClothCuda::Init(char* filename)
{
	...
	d_Vertex = new Vertex(*h_Mesh);
	d_Face = new Face(*h_Mesh);
	d_Hash = new Device_Hash();
	...
	int* tmp;
	CUDA_CHECK(cudaMalloc((void**)&tmp, 10 * sizeof(int)));
...
{
	_strechSpring->IterateConstraint(*d_Vertex);
	_bendSpring->IterateConstraint(*d_Vertex);
}
...
void PBD_ClothCuda::draw(void)
{
	...
	{
		uint ino0 = d_Face->h_faceIdx[i].x;
		uint ino1 = d_Face->h_faceIdx[i].y;
		uint ino2 = d_Face->h_faceIdx[i].z;
		REAL3 a = d_Vertex->h_pos[ino0];
		REAL3 b = d_Vertex->h_pos[ino1];
		REAL3 c = d_Vertex->h_pos[ino2];

		glBegin(GL_POLYGON);

		glNormal3f(d_Vertex->h_vNormal[ino0].x, d_Vertex->h_vNormal[ino0].y, d_Vertex->h_vNormal[ino0].z);
		glVertex3f(a.x, a.y, a.z);
		glNormal3f(d_Vertex->h_vNormal[ino1].x, d_Vertex->h_vNormal[ino1].y, d_Vertex->h_vNormal[ino1].z);
		glVertex3f(b.x, b.y, b.z);
		glNormal3f(d_Vertex->h_vNormal[ino2].x, d_Vertex->h_vNormal[ino2].y, d_Vertex->h_vNormal[ino2].z);
